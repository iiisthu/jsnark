...
public abstract class CircuitGenerator {
	...
	public Wire createInputWire(String... desc) {
		...
		addToEvaluationQueue(new WireLabelInstruction(WireLabelInstruction.LabelType.input, newInputWire, desc));
	...
	public Wire createProverWitnessWire(String... desc) {
		...
		addToEvaluationQueue(new WireLabelInstruction(WireLabelInstruction.LabelType.nizkinput, wire, desc));
	...
	public Wire makeOutput(Wire wire, String... desc) {
		...
		addToEvaluationQueue(new WireLabelInstruction(WireLabelInstruction.LabelType.output, outputWire, desc));
		return outputWire;
	...
	public void addDebugInstruction(Wire w, String... desc) {
		...
		addToEvaluationQueue(new WireLabelInstruction(WireLabelInstruction.LabelType.debug, w, desc));
	}
	...
	public void addDebugInstruction(Wire[] wires, String... desc) {
		for (int i = 0; i < wires.length; i++) {
			...
			addToEvaluationQueue(
					new WireLabelInstruction(WireLabelInstruction.LabelType.debug, wires[i], desc.length > 0 ? (desc[0] + " - " + i) : ""));
		}
	}
	...
	private void initCircuitConstruction() {
		...
		addToEvaluationQueue(new WireLabelInstruction(WireLabelInstruction.LabelType.input, oneWire, "The one-input wire."));
		inWires.add(oneWire);
